...
---@field origin string[] #unique ids associated with nodes where this node originated
...
function M:new(opts)
    ...
    instance.id = opts.id
    -- Normalize origin to always be a string array
    if type(opts.origin) == "string" then
        instance.origin = { opts.origin }
    else
        instance.origin = opts.origin or {}
    end
